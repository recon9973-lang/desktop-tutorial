...
description: >-
  ...
  세션 자동 체크포인트(20건)·핸드오프(방마다 갈린 RESUME), /compact·/clear 안내 훅, CLAUDE.md 지침을
...
2. **세션 체크포인트·핸드오프** — `.claude/skills/checkpoint/` 스킬. 20건 도달 시 문서화→TODO→**이 방 인계본**(`RESUME-<방>.md`) 생성·커밋→`/clear` 안내.
3. **자동 감지 훅** — `.claude/hooks/count-prompts.sh`(UserPromptSubmit: 10건 `/compact` 권장·20건 checkpoint), `.claude/hooks/load-resume.sh`(SessionStart: **지금 가지에 맞는 이 방 인계본 하나**를 자동 주입 — 파일 머리의 `<!-- 가지: claude/... -->` 로 고른다). `.claude/settings.json`에 등록.
4. **CLAUDE.md 지침** — 세션 시작 시 **이 방 인계본**·`PROJECT_STATE.md` 먼저 읽기 + compact/checkpoint 정책.
5. **인계를 방마다 가른다** — 한 저장소에서 방(가지)이 여럿 돌면 인계 이름 하나가 자리 다툼이 된다. 같은 이름에 쓰면 앞 방 것이 조용히 사라지고, 제 이름으로 가르면 훅이 안 읽었다. 그래서 **파일은 방마다 하나(`RESUME-<방>.md`)**, **고르는 열쇠는 이름이 아니라 가지**다.
...
(이 방 인계본·PROJECT_STATE 먼저 읽기 + /compact·checkpoint 정책 + **인계는 방마다·가지로 고른다**). 정본은 `desktop-tutorial/CLAUDE.md` 참조.
...
> 다음 세션부터 **이 방 인계본**·`PROJECT_STATE.md`로 즉시 이어가고, 20건마다 자동 인계됩니다.
