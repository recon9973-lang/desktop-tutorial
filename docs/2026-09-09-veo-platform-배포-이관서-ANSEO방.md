...
> ## ⛔ 끝난 문서다 — 이관은 없던 일이 됐다
>
> 사장님이 **«여기서 배포해»** 로 뒤집으셨고, **이 방이 직접 냈다.**
>
> ```
> veo-platform main 6e9e6684 · 판 0.3.558 · 2026-09-10
> 도는지는 «—» 못 쟀다 (이 방은 운영 주소로 못 나간다)
> ```
>
> **아래 내용은 그때의 기록이다. 지금 따라 하면 안 된다** — 적용 기준
> `37aa650f` 는 두 세대 낡았고, 패치 셋은 지웠다(이미 나간 것을 다시 내게 된다).
> 남은 일은 `RESUME.md` 를 본다.
