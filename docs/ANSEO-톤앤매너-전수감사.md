...
> ✅ **반영 완료 + 배포 완료** — 이 방(desktop-tutorial 세션)이 직접 적용(사장님 지시 «여기방에서 할 것»).
> veo-platform `claude/anseo-console-port` 커밋 `8cd84d4` = **v0.3.390** → main `cd2fe35` 로 나감.
> [실측 2026-08-30 19:21 KST] 운영 서버·워커·웹 셋 다 `version:0.3.390` — **이 41건은 라이브다.**
> 단 하나 유지 판단: «토큰 사용량»은 사장님 어휘(지시 2026-08-25)라 그대로 둠(#35~39 기각).
> §2 감사 #1(체커 실패색)·#3(발행본 등급 부재)은 **v0.3.391(화면 이식 2차 ①)**로 별도 반영.
