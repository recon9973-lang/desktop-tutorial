...
| 4 | M1 토픽 클러스터 | `lib/topic-cluster.js` + 테스트8 + cron 클러스터 모드 + 콘솔 탭 + `?module=cluster` | ✅ 완료 |
| 5 | 통합 | cron 관련글 자동주입(`autoInternalLinks`), admin 사이드바 내비 링크 | ✅ 완료 |
| 6 | QA·후속 | PSI 시계열 연결, posting-settings 토글 UI, 클러스터 자동확장 | ⏳ 후속 |
