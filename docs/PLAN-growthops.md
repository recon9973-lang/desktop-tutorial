...
| 단계 | 모듈 | 산출물 | 상태 |
|---|---|---|---|
| 1 | M2 내부링크 엔진 | `lib/internal-linker.js` + 단위 테스트 + 관리자 리포트 | ▶ 착수 |
| 2 | M3 모니터링 | `api/seo-monitor.js` + 관리자 "SEO 모니터" 탭 | 대기 |
| 3 | M4 아웃리치 CRM | `api/outreach.js` + `outreach.json` + "아웃리치" 탭 | 대기 |
| 4 | M1 토픽 클러스터 | `lib/topic-cluster.js` + cron 클러스터 모드 | 대기 |
| 5 | 통합·QA | 함수 한도 점검, 의료광고법 회귀, 문서화 | 대기 |
