...
**판 번호가 또 겹쳤다.** 「입지 방」이 같은 0.3.496(거래처 「입지」 탭·설정 「데이터 원천」·AI 연결 상태)을
후보 가지 `7dea3e73` 에 올려 CI 초록(run 33772458418, 15:33 UTC). 규칙대로 **먼저 main 에 닿는 쪽을 두고
이 판이 0.3.497 로 물러난다** — 아직 둘 다 main 에 안 닿았으므로 번호는 그대로 두고, 그 방이 닿는 순간
main 위에 다시 얹어 0.3.497 로 올린다(코드 겹침 없음 — 겹치는 것은 판 문자열·계약 문서·changelog·대장·HISTORY 다섯).

**그 방 후보의 대장이 한 곳 낡았다**(기록만, 이 방이 고치지 않는다): §2 에 「미배포 0.3.494~0.3.496」·대기 표에
0.3.494·0.3.495 가 남아 있다. 그 가지는 이 방 도장 커밋 `640866b8`(main) 이전에 갈라졌기 때문이다.
0.3.494~0.3.495 는 [실측 2026-09-03 23:21 KST] 로 이미 나갔다. 그 방 후보는 main 의 조상이 아니라
그대로는 main 에 못 얹히고(비-빨리감기) 합칠 때 이 사실이 드러난다 — 저장소가 스스로 잡는 자리다.
