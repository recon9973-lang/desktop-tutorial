...
- **운영 판 = 0.3.522** (2026-09-07 01:54 KST 이중 실측) · **0.3.523 배포 진행 중**(인구 이동 봉투 꼴 둘 + 0건 응답 앞부분 · 커밋 `40f0cd4` · 실측·도장은 다음 세션이 확인) · 0.3.521 문구 정정 23:42 KST · 0.3.519 인구 이동 22:26 KST · 0.3.517·0.3.518 은 ANSEO 방
...
1. **인구 이동 첫 실호출 확정** — 열쇠는 들어감(2026-09-07 「데이터 원천」 캡처). 0.3.522 뒤 재개방 캡처가
   여전히 「방금 실패해」 = **예외 없이 달 0개** 경로(200 인데 항목 못 읽음) → 행안부 봉투 꼴 `Response.head/items`
   추정 → 0.3.523 이 두 꼴을 읽고 0건이면 응답 앞 300자를 화면에 적음. **다음 세션 첫 일**: 0.3.523 실측 도달
   확인(`/api/health`) → 도장 → 사장님 재개방 캡처의 문장(응답 앞부분) 으로 `location/population_move.py` 의
