...
- **운영 판 = 0.3.519** (2026-09-06 22:26 KST 이중 실측 · 서버·워커 둘 다 · 0.3.517·0.3.518 은 ANSEO 방)
...
1. 공공데이터포털(data.go.kr) 로그인 → 「행정안전부_지역별 인구이동 현황」(15108093) **활용신청** (자동승인).
2. 콘솔 설정 「외부 연결 열쇠」의 **공공데이터포털(심평원) 자리**에 그 계정 인증키가 없으면 넣기 (있으면 끝).
3. 거래처 「입지」 탭 열기 → 인구 카드 맨 밑 「인구 이동 · 최근 12개월」 이 KPI 셋 + 월별 막대로 바뀜.
