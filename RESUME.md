...
- **검증**: 나갈 그 커밋 **`a9de20fa` 에서 preflight 전체 초록**
  [2026-09-08 01:0x KST] — ci-local **7,182** · pnpm -r test **2,259** · test-db ·
  typecheck · lint · build · smoke · 계약 · 대소문자 · 충돌 표시 0 · 미배포 4 커밋 ·
  판 0.3.540 > main 0.3.539. **더 이상 부분 검증인 커밋이 없다.**
  새 시험 11건. 마이그레이션 없음. 잰 값 무변경.
  (CI 예산도 KST 날이 바뀌어 **상한 2회 중 2회 남음**.)
...
1. **배포 — 사장님 지시 «배포는 모아서 하고»** 로 **보류 중**이다. 가지는 초록 상태로 대기.
   ...
   (리베이스했으면 preflight 를 **다시** 돌린다 — 안 움직였으면 위 초록이 그대로 유효하다).
