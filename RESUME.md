...
1. **「나」·「다」가 이 판에 들어갔는지 확인한다.** 그 방은 끝까지 「가」(워드클라우드)만
   검증해 보고했다 — 나(설명 한 줄)·다(8칸+4칸 빈칸 0)는 **«—»** 다.
   **사장님이 화면을 직접 보시는 것이 가장 빠르다** — venom-new-site.vercel.app 의
   AEO 언급현황. 빠져 있으면 한 판 더 필요하다(시안·드롭인은 이미 있다).
2. 한 판 더 필요하면 딸림 방(`session_01QLkftufNw7ecRsE1D5rFWz`)에 넘긴다 —
   **아래 「주의·제약」의 판단 규칙을 먼저 주고** 일을 줘라.
