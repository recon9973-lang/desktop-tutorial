...
1. **배포** — ANSEO 방(세션 「ANSEO 프로그램 스크린샷 분석」)에 **메시지를 직접
   넣어 깨웠다**(2026-08-28 08:56 UTC 발화, trig_01CzQXy7EzhVpK4gPXqQAvSV). 처음에
   문서만 커밋해 놓고 「이관 완료」라고 했던 것은 틀렸다(오류 166) — 받는 쪽이
   모르면 이관이 아니다. 런북은 `docs/ANSEO-배포-인계.md`(이 가지, **열네 판**
   0.3.356~0.3.369 기준). ⚠ 요지: §0 운영 판 먼저 측정 · §3 워커 이미지가 이번부터
   크롬을 싣는다 · §4 배포 뒤 확인 셋(/docs 404 · 진단 창구 404 · 공유 링크 생존).
