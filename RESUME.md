...
- **사장님 몫 하나**(GitHub 설정 — 에이전트가 못 한다)
  - JS 의존성 검사 추가(파이썬 쪽은 `pip-audit` 가 이미 막는다).
- ~~**CI 를 필수 검사로**~~ — **뺐다. 다시 올리지 마라**(사장님 지시 2026-09-10
  «대장에서 빼줘»). ① 이 요금제로는 **못 켠다** — 조직 이전 + Team 결제다
  ([실측 2026-09-10 · 사장님 화면]). ② 그리고 **필요하지도 않다** — main 에 닿는
  길은 `deploy.sh` 하나이고 그 관문은 잘 돈다. 진짜 원인은 **기다리지 않고 병합한
  것**이다(PR #5: 만든 지 **40초** 만에 병합 · CI 는 **8분 49초 뒤**에 끝남).
  처방은 결제가 아니라 **PR 을 main 으로 병합하지 않는 것**이다. 까닭은
  veo-platform 대장 §5 에 남겼다.
