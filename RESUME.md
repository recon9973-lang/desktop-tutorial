...
1. **배포** — 미배포 `0.3.356~0.3.368` **열세 판**. `gh` 있는 방에서 `make deploy`.
   런북은 `docs/ANSEO-배포-인계.md`. ⚠ 이번 판부터 **워커 이미지가 크롬을 싣는다**
   (`infra/docker/worker.Dockerfile`) — 첫 빌드가 길고 이미지가 수백 MB 커진다.
   **빌드가 되는지부터** 본다(이 방에서는 디스크 한도 때문에 못 빌드해 봤다).
