# RESUME — 다음 세션 이어가기 (2026-09-06 13:55 KST · s20 마감 + 0.3.513)

> 새 세션은 이 파일을 **먼저** 읽는다. 이 세션 상세는 `docs/session-logs/2026-09-06-s20.md`.

## 지금 상태

- **운영 판 = 0.3.513** (2026-09-06 13:50 KST 이중 실측 · 서버·워커 둘 다)
- 이 세션이 내보낸 판 넷: **0.3.506** 인구 원천 · **0.3.507** 인구 인포그래픽 · **0.3.509** 교통
  정류장 세부(승/하차/환승) · **0.3.512** 인구 연령 피라미드(5세 21구간 남/여 · 비율 셋) ·
  **0.3.513** 반경 안 인구(행정동 경계 표본점 172,167 · 면적 비례 추정 · 반경 피라미드 · 든 행정동 표)
- 「입지」 탭 세 축 전부 실측값: 경쟁(심평원) · 교통(개수 + 이용량) · 인구(성별·증감 + 연령 피라미드)
- veo-platform 우리 가지 `claude/anseo-location-v0501` = main `86a44f1` + 도장 커밋
...
1. **반경 안 시간대별 생활인구 (서울)** — 조사 끝 (`docs/plans/anseo-floating-population-sources.md`).
   서울 행정동 생활인구(OA-14991 · 열쇠 없음 · 공공누리 1유형) 월 파일을 샌드박스에서 받아
   행정동 × 시간대 × 평일/주말 로 집계 → v0.3.513 표본점 비율로 반경 곡선. 서울 밖은 «—».
2. **인구 이동(전입·전출) 전국** — 행안부 API 15108093 · 사장님 활용신청 1회 + 공공데이터포털 열쇠.
3. **카카오맵 타일** — 사장님이 콘솔 「외부 연결 열쇠」에 카카오 항목 넣으신 뒤 (task #15).
4. **「입지」 이름 겹침 정리** (task #9) · **웹 UI CSP 진단** (task #19) · 전국 유동인구는 SKT 지오비전(유료 · 사장님 결정).
...
- **행정동 경계 표본점**: vuski/admdongkor(CC BY 4.0 · SGIS 출처표시 승계) ver20260701 → 샌드박스 shapely 로 동당 40~62점 → `build_population_points_data.py` 머리말에 방법. 반경 인구는 면적 비례 추정 (화면에 그대로).
