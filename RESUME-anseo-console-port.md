...
## 닫으며 남기는 것 — 이 방이 못 하는 일 하나

**후속 정리는 운영 권한이 있어야 한다. 이 방은 못 한다.**

1. 캡처 계정 `capture@anseo.local`(admin) 삭제 — 화면 대조 때 만든 것이다.
   **운영 데이터베이스에 있고 저장소에는 기록만 있다**(`WORKLIST-HISTORY.md` 7755·7767줄).
2. 화면에 나오는 조직 이름이 실제로 「LOOPEO」인지 확인 — 저장소에서는 **시험 표본에만**
   나온다(`layout.test.tsx` · `session.test.ts` · `medical-review-gate.test.tsx`).

이 방은 운영 주소로 못 나가고(egress 403) 관리자 자격도 없다. **넘겨야 할 일이다.**
