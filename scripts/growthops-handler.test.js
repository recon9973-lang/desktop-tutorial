...
(async () => {
  ...
  await test('indexing: 발행물 기반 준비도 반환', async () => {
    ...
    assert.strictEqual(res._json.ok, true);
    assert.strictEqual(res._json.published, 2);
    assert.ok('missingSlug' in res._json.readiness);
  });

  await test('outreach draft: 연락처 name 없으면 400', async () => {
    process.env.ADMIN_SECRET = 'secret123';
    const res = mockRes();
    await handler(mockReq('POST', { module: 'outreach', action: 'draft' }, { contact: {} }, 'secret123'), res);
    assert.strictEqual(res._status, 400);
    delete process.env.ADMIN_SECRET;
  });

  await test('outreach draft: OPENAI_API_KEY 없으면 500(안전 처리)', async () => {
    process.env.ADMIN_SECRET = 'secret123'; delete process.env.OPENAI_API_KEY;
    const res = mockRes();
    await handler(mockReq('POST', { module: 'outreach', action: 'draft' }, { contact: { name: '대구일보', type: 'pr' } }, 'secret123'), res);
