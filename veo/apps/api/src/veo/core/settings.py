...
class Settings(BaseSettings):
    ...
    #:
    #: **4 → 2 (2026-08-06).** 작업의뢰서 §5.2 가 "동일 호스트 동시 연결 2 이하" 로
    #: 정했다. 4 는 우리가 정한 숫자였고 2 는 남의 서버 기준으로 정한 숫자다.
    console_crawl_concurrency: int = 2

    #: 같은 호스트에 연달아 요청할 때 지켜야 할 최소 간격(초). 의뢰서 §5.2 는 1초다.
    #:
    #: 시간당 총량(`console_target_host_limit_per_hour`)과 **다른 것을 막는다** —
    #: 저것은 한 시간의 총량, 이것은 한순간의 밀도다. 총량만 있으면 450회를 1분에 몰아
    #: 쓸 수 있고, 실제로 그랬다: 2026-08-06 실측에서 venomad.com 131장을 2.5초에
    #: 받아 갔다(장당 0.019초). 그 서버 입장에서는 1초에 50번 두드린 것이다.
    #:
    #: 0 으로 두면 간격을 두지 않는다 — 시험에서만 쓴다.
    crawl_min_interval_seconds: float = 1.0

    #: 한국 관측점(경유) 주소. 비어 있으면 **경유하지 않는다** — 기능이 꺼진 배포다.
    #:
    #: 왜 있는가. 2026-08-05 실측: 운영 서버(싱가포르, 34.21.188.186)가 거래처 4곳 중
    #: 2곳에서 759·760바이트짜리 자바스크립트 쿠키 검사만 받는다. 같은 주소를 한국
    #: 데이터센터(3.37.129.161, AWS 서울)에서 받으면 51,262·64,262바이트가 정상적으로
    #: 온다. 타사(NXT)가 같은 사이트를 문제없이 재는 이유도 그쪽이 AWS 서울이기 때문이다
    #: (ec2-15-164-85-95.ap-northeast-2, AWS 공식 ip-ranges 로 확인).
    #:
    #: **막힌 응답에만** 쓴다. 잘 받아지는 사이트까지 돌아갈 이유가 없다 — 한 걸음이
    #: 늘면 그만큼 느려지고, 고장 날 곳도 하나 는다.
    egress_kr_url: str = ""
    #: 그 관측점을 부를 때 쓰는 열쇠. 없으면 아무나 우리 이름으로 남의 서버를 두드릴 수
    #: 있는 창구가 된다. 값은 환경변수로만 주입하고 코드·로그·화면에 남기지 않는다.
    egress_kr_token: str = ""

    def egress_kr_is_configured(self) -> bool:
        """경유가 실제로 가능한가. 둘 중 하나만 있으면 **꺼진 것으로 본다.**

        주소만 넣고 열쇠를 빠뜨린 배포가 조용히 401 을 받으며 도는 것보다, 처음부터
