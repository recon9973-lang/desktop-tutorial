...
def _field_metric(
    ...
    if not categories:
        ...
        #
        # **문구는 그 사실부터 말한다.** 앞의 판은 "…표본이 없어 … 측정하지 못했습니다"
        # 로 시작했고, 사장님이 그것을 **"CrUX 실패"** 로 읽으셨다(2026-08-09). 읽은
        # 대로다 — "측정하지 못했다" 는 우리 쪽이 뭔가 못 했다는 말이고, 여기서 실제로
        # 일어난 일은 **구글이 아직 이 사이트의 값을 공개하지 않는 것**이다. 우리가
        # 고칠 수 있는 것이 하나도 없는 자리에서 고장처럼 읽히면, 사람은 없는 원인을
        # 찾으러 간다.
        return (
            not_applicable_outcome(
                check_id,
                "구글이 이 사이트의 실사용자(CrUX) 데이터를 아직 공개하지 않습니다. "
                "크롬 방문자가 일정 수를 넘어야 제공되며, 사이트의 결함이 아니고 "
                "점수도 깎지 않습니다. 방문자가 쌓이면 자동으로 평가 대상이 됩니다.",
            ),
            [],
        )
