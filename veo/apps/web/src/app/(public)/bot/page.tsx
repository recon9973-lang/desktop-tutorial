...
const POLICY: readonly { readonly label: string; readonly value: string }[] = [
  // 한 연결이 같은 호스트에 1초에 한 번보다 빨리 요청하지 않고, 연결은 2개까지 연다.
  // 그러므로 서버가 받는 것은 **초당 최대 2회**다. 이 페이지는 우리가 실제로 하는 일을
  // 적는 곳이므로, "최소 1초 간격" 만 적어 두면 초당 2회가 나가는 사실이 가려진다.
  { label: '동일 호스트 연결당 요청 간격', value: '최소 1초' },
  { label: '동일 호스트 동시 연결', value: '2개 이하' },
  { label: '동일 호스트 초당 요청', value: '최대 2회' },
