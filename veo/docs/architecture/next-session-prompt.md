...
> 다음 사람이 있는 것을 새로 만들거나, 없는 줄 알고 건너뛴다.

### 제품

| 순위 | 항목 | 왜 |
|---|---|---|
| 1 | **GEO 관측 실행** (`POST /observations/runs`) | 엔진과 프롬프트 집합은 준비됐다. 실행·저장·지표만 남았다 |
| 2 | 헤드리스 렌더링 | `js_render_parity` 가 실사이트에서 늘 측정 불가 |
| 3 | 추이·회귀 알림 | `RECURRED`·`regression_count` 는 이미 기록 중 |
| 4 | 담당자별 업체 배정 · "누가 무엇을 언제 고쳤나" | 대행사 운영에 필요 |
| ✔ | ~~사이트 전체 크롤 + 병렬 수집~~ | **완료.** 발견 크롤 + 동시 4개 |
| — | 초대 메일 자동 발송 | **의도적 보류** — 링크 직접 전달로 충분하다고 사용자가 결정 |

GEO 관측 실행에 필요한 것은 `docs/architecture/session-log-2026-07.md` 의 "관측 연결
2/2" 에 적어 두었다. 특히 **가격표가 아직 연결되지 않아** 비용이 늘
`NO_PRICE_CONFIGURED` 로 남는다는 점, 그리고 **`gpt-4o-mini`·`gpt-4.1` 로 실행하면
인용 지표가 측정 불가가 된다**는 점을 먼저 읽어라.
