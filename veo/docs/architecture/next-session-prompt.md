...
어긴 것이 남의 코드가 아니라 직전 판에서 내가 쓴 코드였다.

그리고 그 뒤에 0-H·0-I·0-J 가 붙었다. 셋 다 **"고쳤는데 또 나온다"** 는 사용자의 지적에서
나왔고, 답은 "계속 틀린 게 아니라 한 번 정한 것을 끝까지 적용하지 않았다" 였다.
문지기가 없으면 규칙은 기억에 의존하고, 기억은 실패한다.
...
2026-07-31 기준 **4,257 passed · 1 skipped**(건너뛴 1건은 Redis 브로커 미기동).
...
현재 명세: SEO **1.6.0**(57항목), GEO **1.2.0**(37항목, 그중 4항목은 참고).
...
| 순위 | 항목 | 왜 |
|---|---|---|
| 1 | 원문 답변 영속화 (#22) | 재배포하면 근거가 사라진다 → 0-A 가 무너지는 지점 |
| 2 | 키워드 화면 (#25) | 백엔드는 네이버 실 API 에 붙어 있는데 쓸 경로가 없다 |
| 3 | 위험 평가·검수 연결 (#24) | 병원 고객에게 의료 오답 판정은 특히 중요하다 |
| 4 | 반복 관측 시간 분산 (#34) | 같은 시각 3회는 독립 표본이 아니다. 신뢰구간이 실제보다 좁게 나온다 |
...
| ✔ | ~~GEO 콘솔 화면~~ (#19·#31·#33) | **완료.** 관측·준비도 양쪽 + 네이버 참고 조회 |
| ✔ | ~~GEO 를 절대 평가로~~ (#32) | **완료.** 1.1.0 + 모든 명세를 훑는 문지기 |
