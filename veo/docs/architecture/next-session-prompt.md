...
| 순위 | 항목 | 왜 |
|---|---|---|
| 1 | **지표 모듈 중복 정리** (#20) | 직전 판에 내가 `sampling.py` 를 모르고 `metrics.py` 를 새로 썼다. **새 쪽이 0-C 를 더 느슨하게 어긴다** — 1~2회 표본에 퍼센트를 내주고, 비교 5회 기준이 없다. 화면이 이 값을 그리기 전에 |
| 2 | **관측을 비동기로** (#21) | 0-G 위반. 관측 한 번이 수 분이라 **화면을 만들어도 타임아웃으로 끝난다.** `jobs` 테이블은 있는데 쓰는 코드가 0건 |
| 3 | **GEO 콘솔 화면** (#19·#25) | `/console/geo` 는 준비도·관측 **양쪽 다 빈 화면**이다. 제품 이름의 절반이 화면에서 0 |
| 4 | 원문 답변 영속화 (#22) | 재배포하면 근거가 사라진다 → 0-A 가 무너지는 지점 |
| 5 | 키워드 화면 (#25) | 백엔드는 네이버 실 API 에 붙어 있는데 쓸 경로가 없다 |
| 6 | 위험 평가·검수 연결 (#24) | 병원 고객에게 의료 오답 판정은 특히 중요하다 |
| 7 | 헤드리스 렌더링 (#8) | `js_render_parity` 상시 측정 불가 |
| 8 | 추이·회귀 알림 (#9) · 담당자 배정 (#10) | 대행사 운영에 필요 |
