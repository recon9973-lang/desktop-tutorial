...
| 고객이 자기 브랜드를 등록 | **연결됨.** `POST /projects/{id}/brands`. 그전까지 `brand_identities`·`competitors` 는 **쓰는 코드가 0건**이었고, 등록이 없으면 `brand_target_for` 가 거부하므로 GEO 관측이 아예 돌지 않았다 | `test_brands_api.py` |
| 등록 정보 비대칭을 경고 | **연결됨.** 우리 쪽만 채우면 경쟁사 언급이 더 자주 검수 대기로 떨어져 분자에서 빠진다 — 그 상태의 점유율은 노출 차이가 아니라 등록 정보 차이다. 경고하되 저장은 막지 않는다 | `TestBothSidesDescribedTheSame` |
...
### 8.1a 원문 AI 답변이 재배포에서 살아남지 못한다 — ✅ 해소됨

`DatabaseAnswerStore` 가 `answer_documents` 테이블에 보관한다(마이그레이션
`2580c21edf72`). 기본값이 DB 이고, `observation_answer_store=filesystem` 은 개발
장비 전용으로 남겨 두었다.

읽을 때 해시를 다시 계산해 다르면 **읽기를 거부한다** — 바뀐 것을 근거처럼 돌려주는
것보다 못 읽는 편이 낫다. 조직 단위로 갈라지므로 저장 키가 같아도 남의 조직 것은
없는 것이다.

> 원래 우려: `ai_answers` 행에는 저장 키와 해시만 남는다(원문 인라인 금지). 컨테이너
> 파일시스템은 재배포마다 초기화되므로 파일이 사라지면 "이 판정의 근거를 보여 달라"에
> 답할 수 없다. 마스터 0-A 의 마지막 줄이 무너지는 지점이다.

**아직 오브젝트 스토리지가 아니다.** 마스터 §2.1 이 정한 자리는 S3 호환 스토리지이고
그쪽이 맞다 — 원문이 커지면 DB 가 감당할 자리가 아니다. `RecordedAnswerStore` 프로토콜이
그 자리를 이미 정의해 두었으므로 어댑터 하나를 더 만들면 된다.
