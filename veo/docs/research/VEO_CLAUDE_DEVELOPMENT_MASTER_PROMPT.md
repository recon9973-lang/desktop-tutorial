...
### 제품의 목표와 객관성의 기준 (2026-08-01, 사용자 확정)

**목표.** 객관적인 SEO 관계 수치 측정을 통해 고객사 홈페이지를 개선하고, 그것으로
마케팅 효과를 높인다. GEO 도 같은 맥락이다 — 점수는 목적이 아니라 **개선 대상을
특정하고 개선을 증명하는 수단**이다. 화면·검사·문구를 판단할 때의 질문은 언제나
"이것이 고객사 홈페이지의 개선으로 이어지는가"다.

**객관성의 기준은 구글이다.** 가치평가·배점·중요도는 구글의 측정 방법과 공표된
중요도를 기준으로 삼는다. 실행 규칙:

- 근거의 우선순위: ① 구글 실측(Lighthouse `auditRefs[].weight` 직접 판독 —
  `docs/research/LIGHTHOUSE_COMPARISON.md`) ② 구글 공식 문서(developers.google.com·
  web.dev, 수치는 VEO 판단) ③ 구글 근거 없음 — **"VEO 판단"이라고 그렇게 적는다.**
  배점 근거 대조표의 ●◐○ 등급이 이 세 단계다(`SEO_SCORING_V3_PAGES.md` §5).
- 업계 통설을 구글 공식 입장과 섞지 않는다. 섞이면 객관성이라는 약속이 깨진다.
- 구글을 기준으로 삼는 것과 **구글의 결함을 따라 하는 것은 다르다.** 구글은 못 잰
  것을 실패로 적고(0-A 위반), 랜딩 1장만 본다. 기준은 구글의 *가치평가*이지
  구글의 *측정 범위와 실수*가 아니다 — 그 차이가 이 제품의 존재 이유다.
- 구글이 순위 알고리즘 자체를 공개하지 않으므로, 공표된 것과 추정을 항상 구분해
  적는다. VEO 점수는 준비도이지 순위 예측이 아니다(`is_rank_prediction: false`).

### 이 제품의 생명 — 다른 모든 원칙보다 앞선다

아래는 기능이 아니라 **제품이 존재하는 이유**다. 하나라도 어기면 나머지를 아무리 잘
만들어도 그 진단은 쓸 수 없다. 일정에 밀려 넘길 수 있는 항목이 아니고, "일단 이렇게
보이게" 해 두었다가 나중에 고칠 수 있는 항목도 아니다.

| | | 어겼을 때 무슨 일이 일어나나 |
|---|---|---|
| **0-A** | 실측에 대한 정직성 | 재지 않은 것을 잰 것처럼 보고한다 |
| **0-B** | 절대 평가 | 점수가 고객마다 다른 뜻이 되고, 잘못된 유인이 생긴다 |
| **0-C** | 얇은 표본 | 잡음에서 순위를 지어낸다 |
| **0-D** | 있는 것을 다시 만들지 않는다 | 두 벌이 서로 다르게 판단하기 시작한다 |
| **0-E** | 부를 수 없는 기능은 없는 기능 | 완성했다고 세었는데 고객에게는 없다 |
| **0-F** | 초록불은 동작이 아니다 | 통과한 채로 배포가 깨진다 |
| **0-G** | 오래 걸리는 일 | 요청이 끊기고, 비용은 이미 나갔다 |
| **0-H** | 규칙에는 검사를 붙인다 | 규칙이 한 곳에만 적용된 채 몇 달이 지난다 |
| **0-I** | 결함을 지키던 테스트를 찾아라 | 다음 사람이 테스트 이름을 읽고 옛 규칙을 배운다 |
| **0-J** | 못 하는 이유를 고객 탓으로 적지 않는다 | 우리 한계를 고객이 돈으로 풀어야 할 일처럼 넘긴다 |
| **0-K** | 빈 관측을 판정으로 바꾸지 않는다 | 우리가 못 받은 것을 고객 사이트의 결함으로 채점해 내보낸다 |
| **0-L** | 잰 것을 남긴다 | 숫자가 이상할 때 아무도 원인을 확인할 수 없다 |

0-A 와 0-B 는 **처음부터** 이 자리에 있었다. 0-C 부터는 **실제로 그렇게 틀린 뒤에**
올라온 것들이다. 각 항목에 언제 어떻게 틀렸는지를 함께 적어 둔 이유는, 규칙만 적어
두면 다음 사람이 "그럴 리가" 라고 생각하고 같은 자리를 다시 밟기 때문이다.
...
- **분모가 고객마다 달라지면 안 된다.** 연동을 붙일수록 분모가 커져 점수가 내려가면
  **연결하지 않는 편이 유리해진다.** 스키마를 만들수록 채점 항목이 늘어 불리해져도
  마찬가지다. 진단 도구가 만들면 안 되는 유인이다.
  잴 수 없는 영역은 **애초에 점수의 일부가 아니라고 명세에 적고**, 점수를 이루는
  영역만으로 100을 이루게 한다.

  이 하나를 **네 번 틀렸다.** 네 번 다 코드는 정상으로 보였고, 테스트도 전부 초록이었다.
  분모는 조용히 움직이기 때문에 어느 검사도 비명을 지르지 않는다.

  | 언제 | 무엇이 분모를 흔들었나 | 어떻게 드러났나 |
  |---|---|---|
  | 1.1.0 이전 | `UNKNOWN` 이 예산에서 빠짐 | 20점어치를 못 잰 사이트가 **91.8점** (실제 59.1) |
  | 1.3.0 이전 | 연동 영역이 통째로 해당 없음 | **연결할수록 불리** |
  | 1.4.0 이전 | 구조화 데이터가 없으면 12.5점 면제 | **만들수록 불리** |
  | 2026-07-30 | 한 장만 수집하면 다섯 검사가 해당 없음 | **적게 잴수록 유리** (52.23 → 50.11) |

  네 번째는 명세가 아니라 **수집기**에서 났다. 명세를 아무리 지켜도 수집기가
  "우리가 한 장밖에 안 가져왔다" 를 "이 사이트에는 그 항목이 없다" 로 바꿔 적으면
  분모는 똑같이 움직인다. **분모를 지키는 검사를 명세 층과 수집기 층 양쪽에 둔다**
  (`test_denominator_is_fixed.py`).
...
#### 0-C. 얇은 표본에서는 숫자를 내지 않는다

0-A 는 **재지 않은 것**에 관한 규칙이고, 이것은 **조금 재고 많이 말하는 것**에 관한
규칙이다. 둘은 다른 실수이며, 뒤의 것이 더 그럴듯해 보여서 더 위험하다.

AI 답변은 같은 질문에도 매번 달라진다. 세 번 물어 두 번 언급된 것은 "66.7%" 지만,
참값이 20% 일 수도 94% 일 수도 있다. 그 숫자를 경쟁사의 "50.0%" 옆에 소수점까지
찍어 두면 **잡음에서 순위를 지어낸 것**이 된다.

- **모든 비율은 Wilson 95% 신뢰구간을 함께 지닌다.** 정규 근사를 쓰지 않는다 —
  세 번 다 언급된 브랜드에 "100%, 오차 0" 을 주기 때문이다.
- **탐색 최소 3회, 비교 보고 5회.** 3회 미만이면 **퍼센트를 아예 내지 않는다.**
  경고를 붙여서 내보내는 것으로 갈음하지 않는다 — 화면에 뜬 숫자는 읽히고, 주석은
  읽히지 않는다. 3~5회 구간은 측정값이 아니라 **방향**으로 표시한다.
- **관측 0회는 0%가 아니라 데이터 없음이다.** "찾아봤는데 없었다" 와 "안 찾아봤다" 는
  다른 사실이고, 같은 모양으로 그리면 안 된다. `value=null` 과 `value=0.0` 을 화면에서
  구분하지 않으면 **우리가 못 잰 것이 고객 탓으로 보인다.**
- **제공자 실패는 분모에서 뺀다.** 타임아웃은 "언급 안 됨" 이 아니라 "측정 못 함" 이다.
- **비율마다 분모가 다르다.** 인용률의 분모는 유효한 응답 전체가 아니라 **출처를 볼 수
  있었던 응답**이다. 엔진이 출처를 안 밝힌 응답을 분모에 넣으면 인용률이 낮게 나오고,
  그 낮은 값은 사이트 탓처럼 읽힌다.
- **신뢰구간이 겹치면 차이가 아니다.** 두 브랜드의 95% 구간이 겹치는데 "우위" 라고
  쓰지 않는다.

구현은 `veo.observations.sampling` 에 있다. **새로 만들지 말고 그것을 쓴다** (0-D).

#### 0-D. 있는 것을 다시 만들지 않는다

이 저장소는 한 사람이 한 번에 읽기에는 크고, **문서가 실물보다 자주 틀린다.**
"미착수" 라고 적힌 줄을 믿고 새로 만들면 두 벌이 생기고, 두 벌은 반드시 서로 다르게
판단하기 시작한다. 그때 어느 쪽이 맞는지 아무도 모른다.

- **손대기 전에 폴더를 연다.** 문서의 "미착수·없음·Phase N" 을 근거로 삼지 않는다.
  `grep` 한 번이 문서 한 장보다 정확하다.
- **문서와 코드가 다르면 그 자리에서 문서를 고친다.** 다음 사람을 위해 남겨 두지 않는다.
- 이미 이렇게 세 번 틀렸다.

  | 문서가 뭐라고 했나 | 실제 | 대가 |
  |---|---|---|
  | "GEO 관측 미착수" | 약 7,300줄이 이미 있었다 | `router`·`service`·`schemas` 세 파일만 없었는데 전체를 새로 만들 뻔했다 |
  | "네이버 SearchAd 자격증명 없음·미착수" | 실 API 에 연결돼 있고 키도 있다 | 있는 기능을 없는 줄 알고 건너뛰었다 |
  | (아무도 확인 안 함) | `sampling.py` 에 표본 규칙이 이미 있었다 | `metrics.py` 를 중복으로 만들었고, **새 쪽이 규칙을 더 느슨하게 어겼다** |

  마지막 것이 이 규칙이 존재하는 이유다. 중복은 낭비로 끝나지 않는다. **나중에 만든
  쪽이 원본의 제약을 모른 채 더 관대해진다.**

#### 0-E. 부를 수 없는 기능은 없는 기능이다

테스트가 통과하는 모듈과 제품이 쓰는 모듈은 다르다. 완성된 엔진에 **호출자가 없으면
고객에게는 존재하지 않는다.** 그런데 진행률 보고에서는 완성으로 세어진다 — 그래서 이
착각은 스스로 커진다.

- 기능을 완료라고 말하기 전에 **`src/` 안에서 그것을 부르는 코드**를 찾아라.
  테스트만이 부르고 있다면 미완료다.
- 화면까지 닿았는지 본다. **API 는 있는데 화면이 없으면 절반이다.**
- 이 착각으로 남아 있던 것들: 관측 실행기(612줄, 호출자 없었음), 위험 평가·사람 검수
  (`claim_assessments` 에 쓰는 코드 0건), Celery 워커(전부 스텁, `.delay()` 호출 0건),
  SOV(계산은 완성, 실측과 연결 없음).

#### 0-F. 초록불은 동작이 아니다

- **웹은 `next build` 까지 돌린다.** 타입체크·린트·단위 테스트가 모두 통과해도 빌드는
  깨진다. 실제로 Next 16 의 `middleware`→`proxy` 규약 변경을 모르고 빌드를 깨뜨렸는데,
  배포는 Error 로 끝났고 화면은 옛 코드로 남아 **사용자가 물어보고서야 알았다.**
- **API 테스트는 DB 를 붙여 돌리고 `passed` 숫자를 확인한다.** `VEO_TEST_DATABASE_URL`
  없이 돌리면 DB 테스트가 전부 SKIP 되는데 종료 코드는 0 이다. **건너뜀은 통과가 아니라
  공백이다.**
- **화면을 고쳤으면 렌더해서 실제로 잰다.** 스크린샷을 보고 추측하지 않는다.
- **하위 에이전트·워커의 보고를 그대로 믿지 않는다.** 루트가 직접 diff·테스트·계약을
  확인한다. 실제로 여러 번 틀렸다.
- 마지막 판들에서 **테스트보다 "그려 보고·재 보고·배포해 보고" 잡은 결함이 더 많았다.**

#### 0-G. 오래 걸리는 일은 요청 안에서 하지 않는다

관측 한 번은 프롬프트 × 엔진 × 반복(최소 3회)만큼 외부 AI 를 부른다. 사이트 크롤은
수십 장을 가져온다. 이런 일을 HTTP 요청 안에서 그대로 돌리면 **게이트웨이가 먼저
끊고**, 사용자에게는 "기능이 고장났다" 로 보인다. 그 시점에는 이미 비용도 나갔다.

작업은 `jobs` 에 기록하고 워커로 넘긴다. 진행률·부분 성공·취소를 함께 만든다(§5).

#### 0-H. 규칙을 정했으면 그 규칙을 지키는 검사를 함께 만든다

**이 항목은 앞의 모든 항목을 실제로 작동하게 만드는 장치다.**

절대 평가는 `veo.seo.readiness/1.2.0` 에서 **한 번** 정해졌다. SEO 는 전부 옮겼다.
GEO 만 안 옮겨졌고, **다섯 달 동안 아무도 몰랐다.** 이유는 하나다 — 테스트 전체에서
그 규칙을 검사하는 코드가 **0건**이었다. 규칙은 이 문서에 적혀 있었고, 사람이 기억으로
지켜야 했고, 기억은 실패했다.

- **개별 사례를 고치고 끝내지 마라.** "여기도 그런가" 를 묻고, 그 물음을 검사로 바꾼다.
- 검사는 **앞으로 생길 것까지** 걸러야 한다. 발행된 모든 명세를 훑는 검사는 다음에
  만들 키워드·경쟁사 명세도 잡지만, `veo.geo.readiness` 를 콕 집은 검사는 못 잡는다.
- **탈출구가 하나뿐인 규칙은 그 탈출구를 감사한다.** 절대 평가에서 분모를 빠져나가는
  길은 `해당 없음` 하나뿐이다. 그 하나를 수집기마다 전수로 읽어 보면, "우리가 못 잰
  것" 을 거기로 흘려보내는 자리가 나온다. 실제로 SEO 에서 두 곳, GEO 에서 두 곳 나왔다.

#### 0-I. 결함을 고칠 때, 그 결함을 지키고 있던 테스트를 찾아라

고친 뒤 테스트가 깨지면 대개 기뻐할 일이다. 그런데 이 저장소에서는 **깨진 테스트가
옛 규칙을 이름으로 못 박고 있던 경우**가 반복해서 나왔다.

    test_unknown_lowers_coverage_rather_than_the_score     ← 상대 평가를 고정
    test_no_sitemap_means_the_check_does_not_apply         ← 못 잰 것을 해당 없음으로
    test_a_single_page_crawl_cannot_judge_duplication      ← 같은 것
    test_a_rate_below_the_floor_is_not_reportable          ← 얇은 표본에 값을 주는 쪽

이런 테스트는 **통과하도록 값만 바꾸면 안 된다.** 이름이 곧 주장이고, 그 주장이
틀렸다. 이름째 바꾸고, 왜 반대가 맞는지를 docstring 에 적는다. 그러지 않으면 다음
사람이 그 이름을 읽고 옛 규칙이 옳다고 배운다.

#### 0-J. 못 하는 이유를 고객 탓으로 적지 않는다

0-A 의 한 갈래인데, 라벨 하나로 조용히 어긋나므로 따로 적는다.

GEO 외부 검증 네 항목을 "유료 데이터원이 필요합니다" 로 적어 두었다. 사실이 아니었다 —
이미 가진 네이버 자격증명으로 조회가 그대로 열렸다. 고객은 그 문장을 읽고 **자기가 돈을
더 내야 한다**고 이해한다. 실제로는 우리 쪽 한계였다.

- 점수 밖 항목마다 **누구 때문에 밖인지**를 정확히 적는다.
  `CUSTOMER_GRANTED`(고객이 권한을 줘야) · `PAID_PROVIDER`(유료 데이터원) ·
  `REFERENCE_ONLY`(조회는 되지만 우리가 그 값을 믿지 못한다).
- 편한 칸에 몰아넣지 마라. 한 번에 네 항목을 같은 라벨로 찍었다가 셋이 틀렸다.
- **점수 밖이라고 화면에서 빼지 마라.** 빠지면 "그런 항목이 없다" 로 읽힌다. 하단에
  사유와 함께 남기고, 조회할 수 있는 것은 참고 수치까지 보여준다.

#### 0-K. 빈 관측을 판정으로 바꾸지 않는다

0-A 를 어긴 방식이 하나 더 있어서 따로 적는다. 0-A 는 "값을 지어내지 마라" 인데,
여기서 지어낸 것은 **값이 아니라 부재** 였다. 아무것도 못 받아 놓고 "그 사이트에는
그것이 없다" 고 적었다.

2026-08-04, venomad.com 진단이 22.7~27.3 점으로 나왔다. 타사 도구는 94 점이었다.
실측으로 확인한 것:

- 운영 서버는 그 주소에서 **HTTP 200 을 받았다.** `status_ok` 는 통과했다.
- 그런데 본문에 title·h1·lang·viewport·charset·canonical·JSON-LD 가 **하나도 없었다.**
- 엔진은 이것을 측정 실패로 보고하지 않고 **11개 항목을 FAIL 로 채점**했다.
- 같은 주소를 다른 곳에서 받으면 그 값들이 **전부 멀쩡히 있다.** 같은 엔진을
  로컬에서 돌리면 75.9 점이 나온다.

즉 우리는 **받지 못한 것을 고객 사이트의 결함으로 바꿔** 고객 화면에 내보냈다.
"측정 불가" 라고 적을 자리에 "당신 사이트에 제목이 없습니다" 라고 적은 것이다.

**규칙**

- 응답 코드만으로 수집 성공을 판단하지 않는다. **200 은 "서버가 답했다" 이지
  "우리가 문서를 받았다" 가 아니다.**
- 문서가 문서의 꼴을 갖추지 못했으면(예: `<head>` 안의 필수 요소가 통째로 없고
  본문도 비어 있다) 그것은 **그 사이트의 상태가 아니라 우리 수집의 상태**다.
  이때의 올바른 출력은 점수가 아니라 **수집 실패**이며, 무엇을 얼마나 받았는지를
  함께 남긴다.
- 부재를 판정하기 전에 **부재를 판정할 자격이 있는지** 먼저 확인한다. 받은 바이트
  수, 파싱 성공 여부, 최소한의 문서 구조 — 이 셋이 확인되지 않은 문서로는 어떤
  "없음" 도 확정하지 않는다.
- 이 규칙에는 **검사를 붙인다**(0-H). 규칙이 이미 0-A 로 적혀 있었는데도 어긴 이유는
  검사가 없어서 어긴 줄을 아무도 몰랐기 때문이다. 빈 문서·잘린 문서·차단 페이지를
  넣었을 때 점수가 나오지 않고 수집 실패가 나오는지 확인하는 시험을 둔다.

**왜 이것이 제품의 존폐인가**

이 도구는 측정기다. 측정기가 "재지 못했다" 를 "나쁘다" 로 번역하기 시작하면, 그
숫자는 사이트에 대한 정보가 아니라 우리 네트워크 사정에 대한 정보가 된다. 고객은
그것을 모르고 자기 사이트를 고치려 든다.

#### 0-L. 잰 것을 남긴다

**이 제품은 진단기다. `측정 → 알고리즘(진단) → 점수` 순서로 이어진다**(사장님 정의,
2026-08-04). 이 순서가 이 규칙의 전부를 정한다.

- **측정 층은 순수해야 한다.** 관측한 사실만 들어간다. 여기에 추론이 섞이면 뒤의 모든
  것이 오염되고, 오염된 것을 알아챌 방법이 없어진다.
- **알고리즘(진단) 층은 판단해도 된다.** 그것이 이 층의 일이다. 다만 그 판단은 발행
  명세에 근거와 함께 선언되어야 한다 — 코드에 박힌 숫자는 판단이 아니라 사고다.
- **우리 자신에 대한 사실은 어느 층에도 들어가면 안 된다.** 우리가 얼마나 확신하는지,
  우리가 몇 장을 쟀는지는 고객 사이트에 대한 사실이 아니다. 그것이 점수를 바꾸면
  숫자는 사이트가 아니라 우리 사정을 보고하게 된다.

**그리고 측정한 것을 버리지 않는다.**

2026-08-04, venomad.com 진단이 15~27점으로 나왔다. "무엇을 받았길래 그 점수인가" 에
답할 방법이 없었다. 남아 있던 것은 sha256 해시 64자와 수집기가 고른 2,000자 발췌뿐이었고,
둘 다 사람이 열어 볼 수 있는 것이 아니었다. 코드를 읽고 판정에서 거꾸로 추측하는 데
감사관 넷과 하루가 들었고, 그러고도 확정하지 못했다.

사장님의 지적이 정확했다 — **아무 AI에게 주소를 주면 페이지를 열어 보고 답한다.**
우리는 진단을 위해 만든 프로그램인데 측정한 것을 안 갖고 있었다.

- 진단마다 **받은 응답을 그대로 보관한다.** 판정도 요약도 아니고 원자료다.
- **못 읽은 응답을 먼저** 담는다. 잘 읽은 페이지는 점수가 설명해 주지만, 못 읽은
  응답은 아무것도 말해 주지 않는다.
- 상한을 두되 **잘린 것을 전부인 척하지 않는다.** 원본 크기와 잘림 여부를 함께 남긴다.
- **꺼내 볼 수 있어야 한다.** 저장만 하고 부를 창구가 없으면 없는 기능이다(0-E).
- 쿠키·인증 헤더는 **담을 목록을 정해 두고 목록 밖은 버린다.** 지울 목록을 정하면
  새 헤더가 생길 때마다 새어 나간다.

**설계는 알고 있었는데 배선이 없었다.** `evidence.storage_key`("큰 자료는 객체
저장소에")·`TargetView.was_fetched`·`FetchedDocument.truncated`·`unread_documents` —
넷 다 선언만 있고 채우거나 읽는 코드가 없었다. 필요한 것을 아는 것과 이어 두는 것은
다른 일이고, **이어지지 않은 필드는 없는 것과 같다.**
