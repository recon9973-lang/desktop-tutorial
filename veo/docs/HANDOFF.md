...
### 가. #45 한도를 화면에 붙인다 — **지금 가장 급하다**

기록은 쌓이고 세는 법도 있는데 **읽을 곳이 없다.** 한도(하루 25,000회)에 다가가는 것을
아무도 모르고, 넘고 나서야 "성능이 전부 측정 불가" 로 드러난다 — 그것도 화면에는
**사이트의 문제처럼 보이는 형태**로.

순서: 엔드포인트 → `/console/usage` 화면 → openapi·TypeScript 클라이언트 재생성 → 경보.
`/observations/spend` 가 본뜰 패턴이고, 문구는 `QuotaUsage` 에 이미 있다.

경보는 화면과 별개다. 화면은 사람이 봐야 알고, 한도는 사람이 안 볼 때 넘는다.
...
### 한도 — 세는 법은 있고 볼 곳이 없다 (작업 #45)

`veo.usage.pagespeed_quota(db, organization_id=...)` 가 오늘 몇 번 불렀고 몇 번
남았는지 답한다. **아직 부르는 곳이 없다** — 0-E: 부를 수 없는 기능은 없는 기능이다.
