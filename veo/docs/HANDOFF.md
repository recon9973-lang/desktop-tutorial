...
### 이번 세션에 한 일 (커밋 6개, 브랜치는 원격에 올라가 있음)
...
### 나. #37 구글 400 오분류 — 고객이 조치할 수 없는 문구
...
`apps/api/src/veo/providers/google/errors.py:205` `classify_status` 가 401/403/429/5xx
만 분기한다. **네이버 쪽 같은 이름의 함수에는 4xx 분기가 이미 있다**
(`providers/naver/errors.py:241`) — 거기 주석이 이유를 적어 두었다.

### 다. #39 "구글은 SEO 100점 주는데" 영업 근거 정리
...
문제를 찾았다(한 페이지에는 canonical 이 2개). **구글이 틀린 게 아니라 본 페이지 수가
다르다** — 이 표현을 그대로 쓸 것.

### 라. #38 에이전트형 브라우징 — 제품 판단 필요

구글이 Lighthouse 에 `AGENTIC_BROWSING` 카테고리를 만들었다(개발 중이라 명시).
배점 있는 항목은 실질 2개(접근성 트리, CLS)뿐이고 **llms.txt·WebMCP 는 배점 0** 이다.
VEO GEO 준비도와 겹치므로 판단이 필요하다. 상세는 작업 #38 설명.

### 마. #8 헤드리스 렌더링 — `js_render_parity` 를 실제로 측정
