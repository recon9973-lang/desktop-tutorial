...
- **버전 규칙 (2026-08-03 사용자 요청: "버전마다 모두 저장 — 그래야 구분 가능")**:
  사용자 눈에 보이는 변경이 쌓여 한 판이 되면 `apps/web/src/lib/changelog.ts` 맨 앞에
  항목을 추가하고(그 버전이 곧 APP_VERSION), `apps/api/src/veo/__init__.py` 의
  `__version__` 을 같은 값으로 맞춘다. 콘솔 하단 푸터의 "v… 변경 이력" 이 이 값을
  보여 주고 `/console/changelog` 로 연결된다.
...
- 간편 진단(site 미지정)은 동기 경로 유지.

**P1-7a 완료(2026-08-03): 진단 간 이슈 diff + 통합 회귀 경보.**
- `maybe_alert_score_drop` → `maybe_alert_regression` (0-I 이름째 교체): 점수 하락
  **또는** 새 이슈 등장이면 **한 건**의 메시지(두 통이면 채널이 꺼진다). 본문에
  점수 변화 + 새로 생긴 이슈(제목 3건까지+외 N건) + 고쳐진 이슈 수.
...
**P2-11 완료(2026-08-03): 의료광고법 원고 검수기 — 위반 판정이 아니라 검토 신호.**
- `veo/medical/`(rules·review·router): 의료법 제56조 2항 유형 9종(치료경험담·효과
  보장·최상급·비교비방·전후비교·가격 이벤트·근거 불명 수치·신의료기술 암시·기사
  가장) + **부재형 규칙**(시술 원고에 부작용·개인차 언급이 없으면 그 사실을 표시,
  있으면 침묵). 규칙마다 근거 조항(reference_ko) 필수 — 근거 못 대는 규칙은 안 넣음.
- **점수 없음**(필드 구조를 시험이 고정 — 숫자가 생기는 순간 "몇 점이면 안전" 읽기),
  모든 응답에 면책("법률 판단이 아니다·심의 확인") 동봉, 원고는 저장하지 않음.
- `POST /medical/copy-reviews`(scan:read) + 콘솔 /console/medical(원고 붙여넣기 →
  유형·조항·걸린 구절·확인할 일 목록). 발견 0건 문구는 "적법"이 아니라 "표시할
  것을 찾지 못함". 규칙 확장은 rules.py 한 곳 — 심의 지적 사례가 쌓이면 추가.
