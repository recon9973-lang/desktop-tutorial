...
2. **컨텍스트 관리**: 세션이 길어지면 **`/compact`**(연속성 유지·압축)를 사용자에게 권한다. 질문/오더 **20건 도달** 시 **`checkpoint` 스킬** 실행(문서화→TODO→`RESUME.md` 생성→커밋) 후 사용자에게 **`/clear`** 입력을 안내한다.
   - **훅(`UserPromptSubmit`)이 10·20건에서 자동 알림**하지만, **웹/원격 환경에선 훅이 안 돌 수 있다**(`/hooks` 미지원). 그 경우 **에이전트가 스스로 세션의 질문/오더 수를 세어** 20건에 도달하면 사용자 지시 없이 **자동으로 checkpoint를 실행**하고(문서화·TODO·RESUME 저장·커밋까지 전부 완료) **그다음 "`/clear` 입력하세요"만 안내**한다. 즉 트리거가 훅이든 자기 카운트든, 사용자가 별도 오더하지 않아도 20건에서 인계가 준비돼야 한다.
