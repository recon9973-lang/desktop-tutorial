...
2. **컨텍스트 관리**: 세션이 길어지면 **`/compact`**(연속성 유지·압축)를 사용자에게 권한다. 질문/오더 **20건 도달** 시 **`checkpoint` 스킬** 실행(문서화→TODO→`RESUME.md` 생성→커밋) 후 사용자에게 **`/clear`** 입력을 안내한다. (`UserPromptSubmit` 훅이 10·20건에서 자동 알림)
