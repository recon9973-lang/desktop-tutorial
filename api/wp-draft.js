...
module.exports = async function handler(req, res) {
  ...
  const { siteUrl, user, appPassword, title, html, excerpt, status, date } = req.body || {};
  try {
    // 기본은 임시저장(draft). 예약(future)·즉시발행(publish)은 명시적으로 요청한 경우에만.
    const st = ['draft', 'publish', 'future'].includes(status) ? status : 'draft';
    const r = await createWpDraft({ siteUrl, user, appPassword, title, html, excerpt, status: st, date });
