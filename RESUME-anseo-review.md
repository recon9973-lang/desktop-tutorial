# RESUME (검수 방 · ANSEO 96항목) — **닫힘. 전부 나갔다** (2026-09-26 확인)

> **이 방은 닫는다.** 아래는 닫기 전에 [실측 2026-09-26]으로 다시 확인한 것이다.
>
> ```
> 이 방 커밋 넷        전부 origin/main 의 조상 (b4b250b5 · d9797772 · f3f22107 · a0767a63)
> 이 방이 넣은 코드     main 에 살아 있다 — 맥락 robots 차단 · 부재 문장 도우미 ·
>                      NOT_CRAWLED · claim_version 가지 가르기
> 이 방 발행본          GEO 1.10.0 · SEO 1.15.0 — 운영에서 **가장 높은 판**으로 돌고 있다
> 대장 대기 표          비었다 (main 머리말 「미배포 없음」) — 이 방 줄 0건
> 열린 PR               이 방 것 없음 (#25 는 다른 방 가지)
> 가지에 남은 것        도장 커밋 하나(3fc54443) — **저쪽이 이미 같은 도장을 찍어 무의미**
>
> [실측 2026-09-26 13:38 KST · 러너 「도는 판 확인」 실행 36218411672]
>   화면 · 진단 서버 · 워커 1대   셋 다 0.3.657 · 뒤처진 워커 0
>   발행본                        GEO 1.10.0(11개) · SEO 1.15.0(18개)
> ```
>
> **남은 것은 이 방이 못 하는 것 셋뿐이다**(아래 「대기/차단 — 사장님 몫」).
> 다시 열 일이 생기면 이 파일과 `docs/session-logs/2026-09-*-anseo-review.md` 를 읽는다.
...
## 바로 앞에 나간 것 — **부재를 말하는 문장이 자기 범위를 말한다** (판 0.3.619 · 나갔다)

사장님 «이어서 진행» 2026-09-19. 이번 판에서 **미뤄 둔 이웃 구멍**을 재 봤더니
이 방이 있는 까닭 그대로의 모양이 또 있었다 — **같은 관측이 한 화면에서 두 번, 서로 반대로.**
...
수집은 자기가 무엇을 안 봤는지 **알고 있었고 화면에 적고 있었다.** 그런데 그 사실이
`CollectionContext` 에 안 실려 **수집기는 몰랐다.** 이제 맥락이 그 주소들을 나르고,
부재를 말하는 **네 문장**이 막힌 주소 수를 함께 적는다.

**점수는 한 점도 안 움직인다** — [실측] 막은 쪽 41.046 · 안 막은 쪽 37.406, 이 판
전후로 둘 다 같다. robots 로 막힌 주소는 **검색엔진도 크롤하지 않으므로** 그 자리의
중복은 검색에 닿지 않는다 — **막는 것은 트릭이 아니라 조치다.** 우리 200장 한도와
다른 점이 여기다(그쪽은 사이트가 아무것도 바꾸지 않았다). 진단 못 함으로 내리지도
않는다 — 내리면 `/admin/` 을 막아 둔 멀쩡한 사이트가 벌을 받는다(0-A 의 반대편).

커밋 `b4b250b5` · 판 `d9797772` 로 나갔다(사장님 «내보내») · 도장 `3fc54443`.
시험 9 · 반증 확인 · API 전체 초록.

```
[실측 2026-09-19 13:44 KST · 러너 실행 35422112453]
  화면 · 진단 서버 · 워커 1대   셋 다 0.3.619 · 뒤처진 워커 0
  발행본                        GEO 1.10.0(11개) · SEO 1.15.0(18개)
```

⚠️ **합치기 전에 겹쳤는지 먼저 세라.** main 의 마지막 커밋이 「같은 구멍을 고친 두 판
중 이 방 것을 접는다」였다 — 방이 여럿이면 같은 자리를 둘이 고친다. 이번엔 안 겹쳤고
(`git show origin/main:<파일>` 로 셌다), **겹쳤는지는 세어 보고 말한다.**
