...
1. **워커가 0.3.575 인지 확인** — 사장님 캡처로 `veo-platform`(진단 서버)은 **0.3.575 ACTIVE ·
   Deployment successful** 확인됨. **`veo-worker` 는 아직 못 봤다.** 진단을 실제로 돌리는 것은
   워커이므로, 워커가 옛 판이면 이번에 나간 것(GEO 1.8.0 39항목 · 크롤러 로스터)이 **실제
   진단에 안 먹는다.** Railway → `veo-worker` → Deployments 에서 같은 커밋이 ACTIVE 인지 본다.
   (이 방은 운영 주소에 못 닿는다 — egress 403. 캡처나 콘솔로만 잰다.)
