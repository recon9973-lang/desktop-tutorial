...
## 지금 상태 — 판 둘이 나갔다 (0.3.581 · 0.3.582)

```
[실측 2026-09-15 · git]
veo-platform       main d6aa9085 = 0.3.582 · 내 커밋 전부 main 의 조상
desktop-tutorial   미커밋 0 · main 보다 2커밋 앞(검수 보고서 정정 · 이 인계)
...
**도는 판을 쟀다 — 러너를 시켜서**(사장님 오더 «니가 확인해줘»). 이 방은 운영 주소에
못 나가지만(프록시가 「policy denial」로 CONNECT 403) **GitHub 러너는 나간다.**
`rollout-check.yml` 을 `mcp__github__actions_run_trigger` 로 부른다 — 옛 인계에 「403」이라
적혀 있지만 그것은 `gh` 이야기고, **MCP 도구로는 뜬다.**

```
[실측 2026-09-15 17:59 KST · 러너]
  /api/health         200   0.3.582 · production · 07:34:14Z 에 떴다
  /api/queue          200   워커 1대 · ["0.3.582"] · stale 0
  /api/scoring/specs  200   GEO 가장 높은 판 1.9.0 · SEO 1.14.0   ← 발행본이 이것이다
  커밋 07:22:53Z → 서비스 07:34:14Z  (11분 뒤에 떴다 = 같은 코드다)
```

**그래서 ① 은 끝났다** — 0.3.582 가 돌고 GEO 1.9.0 이 발행본으로 골라진다.
**다만 옛 리포트의 숫자는 안 바뀐다** — 발행본은 불변이고(ADR 0012) 리포트는 자기가
매겨진 판을 들고 있다. 막힌 거래처의 **옛 리포트는 여전히 95.062**, **다음 진단부터
0.000** 이다. 「열었는데 그대로다」는 고장이 아니다.
