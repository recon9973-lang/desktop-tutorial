...
신한라이프 GA 업무 반자동화. **원스톱 웹앱** 하나로 ①동의서 작성과 ②웹 폼 입력을 모두 사용.

## ★ 원스톱 웹앱 (권장 인터페이스)
정보를 한 번 입력(사진 OCR·카톡 붙여넣기·직접)하면, 한 화면에서 ①과 ② 둘 다 사용한다.
```bash
python -m webapp.server      # http://127.0.0.1:8000
```
![원스톱 웹앱](webapp/preview_onestop.png)

- **1단계 공통 입력**: 사진 드래그드롭(온디바이스 OCR)·카톡 붙여넣기·직접입력 → 이름·전화·주소·주민번호(마스킹·체크섬) 자동 정리.
- **① 가입설계동의서**: 원본 양식 PDF 업로드 → 손글씨 기재 + 펜 체크 → 미리보기·PDF·팩스이미지 다운로드.
- **② 신한 웹 폼 입력**: 검증 후 GA 반자동 입력(로그인·간편PW 직접, 제출 전 확인). *실제 사이트 셀렉터 캘리브레이션 후 활성화.*
- 개인정보 보호: **주민번호 원문은 브라우저로 보내지 않고 서버 메모리에서만** 처리.

> 데스크톱 앱(PySide)도 유지됨(`python -m ui.app_ui`). 아래는 엔진/CLI 구조.

---

**두 프로젝트를 별개 엔진으로** 구현(웹앱·데스크톱·CLI가 공유).
