...
const { buildToneDirective } = require('./tone-engine');
...
// 본문 ⑧ FAQ 섹션(<h2>자주 묻는 질문…</h2> 아래 <h3>질문</h3><p>답변</p> 반복)에서
// Q&A 쌍을 추출해 schema.org FAQPage(mainEntity) JSON-LD를 생성한다.
// Google FAQ 리치결과·AI Overview 채택 신호. Q&A 2개 미만이면 null(스키마 생략).
...
function buildFaqSchema(html) {
  if (!html) return null;
  // FAQ 섹션 시작(자주 묻는 질문 또는 FAQ가 든 h2)부터 다음 h2 전까지로 한정
  const h2re = /<h2\b[^>]*>([\s\S]*?)<\/h2>/gi;
  let m, start = -1, end = html.length;
  const heads = [];
  while ((m = h2re.exec(html))) heads.push({ idx: m.index, after: h2re.lastIndex, text: stripTags(m[1]) });
  for (let i = 0; i < heads.length; i++) {
    if (/자주\s*묻는\s*질문|FAQ|자주하는\s*질문/i.test(heads[i].text)) {
      start = heads[i].after;
  ...
  // <h3>질문</h3> 바로 뒤의 <p>답변</p> 쌍 추출
  const qa = [];
  const pairRe = /<h[34]\b[^>]*>([\s\S]*?)<\/h[34]>\s*(?:<p\b[^>]*>([\s\S]*?)<\/p>)?/gi;
  let p;
  while ((p = pairRe.exec(section))) {
    // 접두 라벨 제거는 명확한 구분자(. : ) )가 뒤따를 때만 — "AEO/AI" 같은 본문 훼손 방지
    let q = stripTags(p[1]).replace(/^\s*(?:Q[.):]\s*)?(?:\d{1,2}[.):]\s*)?/i, '').trim();
    let a = stripTags(p[2] || '').replace(/^\s*A[.):]\s+/, '').trim();
    if (q.length < 4 || a.length < 10) continue;       // 너무 짧은 항목 제외
    if (!/[?？]$/.test(q)) q = q + '?';                  // 질문은 물음표로 정규화
    qa.push({ q, a });
  }
  ...
  return JSON.stringify({
    ...
    '@type': 'FAQPage',
    'mainEntity': qa.slice(0, 10).map(({ q, a }) => ({
      '@type': 'Question',
      'name': q,
...
async function generatePost({ category, keyword, region = '', extra = '', target = '', tone = '', detail = '' }) {
  const catLabel = CAT_LABEL[category] || category;
  // 원고작성 프로젝트 톤 레이어: target/tone이 있으면 "다정한 구어체+문어체" 지시 주입(없으면 ''=기존 동작)
  const toneBlock = buildToneDirective({ target, detail, tone });
  ...
  try {
    ...
    // 같은 키워드로 여러 글을 쓸 때 글마다 다른 연관어·질문이 앞에 오도록 회전(중복 방지)
    const rotSeed = hashSeed((keyword || '') + '|' + (region || '') + '|' + Date.now());
    researchBlock = buildResearchPrompt(research, rotSeed);
...
${region ? `'${region}' 지역 맥락(상권·검색 패턴·지역 키워드)을 구체적으로 반영할 것.` : ''}${toneBlock}
...
${DELIMITER.END}`;
  ...
  const faqSchema = buildFaqSchema(parsed.html);
  if (faqSchema) {
    const n = (JSON.parse(faqSchema).mainEntity || []).length;
    console.log(`[faq-schema] ${n} Q&A → FAQPage JSON-LD 생성`);
  }
...
module.exports = { generatePost, CAT_LABEL, buildFaqSchema };
