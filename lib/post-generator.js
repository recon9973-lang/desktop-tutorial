...
// ── 시스템 프롬프트 (압축 버전: ~280 토큰) ──
const SYSTEM_PROMPT = `병원마케팅 에이전시 베놈(VENOM) 블로그 라이터. GEO/AEO/SEO 고품질 콘텐츠 작성.

구조 필수:
①역피라미드: 첫 문장=핵심결론+수치(~입니다 종결) ②Executive Summary <h2>핵심 요약</h2>+<ul>인사이트3개 ③<h3>질문형 소제목 7개+답변(수치포함) ④<table>비교표1개 ⑤<h2>자주 묻는 질문(FAQ)</h2>+7개Q&A ⑥베놈CTA 1회

분량: 순수텍스트 2500~3500자, <h2>5개+, <h3>7개+, <ol>/<ul>3개+
...
${DELIMITER.HTML}본문HTML(2500~3500자,h2/h3/table/ul/ol 필수)
${DELIMITER.END}`;

  const raw = await chatComplete(SYSTEM_PROMPT, userPrompt, {
    model: process.env.OPENAI_TEXT_MODEL || 'gpt-4o-mini',
    max_tokens: 3200,  // 한글 3500자 ≈ 2500토큰 + HTML태그 700
    temperature: 0.68,
  });
