...
function cleanContent(html) {
  ...
  // 5) 가짜 전화번호 제거 (GPT 환각 방지) — '전화: 010-1234-5678' 등
  //    라벨+번호를 '카카오 상담'으로, 잔여 번호는 삭제
  out = out.replace(/(전화|연락처|문의(?:전화)?|상담전화|대표번호|고객센터|Tel\.?|TEL\.?|tel\.?)\s*[:：]?\s*(?:\+?82[-\s]?)?(?:0\d{1,2}|01\d)[-\s.]?\d{3,4}[-\s.]?\d{4}/g, '카카오 상담');
  out = out.replace(/(?:\+?82[-\s]?)?0\d{1,2}[-\s.]\d{3,4}[-\s.]\d{4}/g, ''); // 구분자 있는 전화형 번호
