...
async function generateAndSaveImage(prompt, postId, index = 0, title = '') {
  ...
  let dalle = await callDallE(apiKey, fullPrompt);
  // 폐기/오타 모델(예: dall-e-3) 자동 폴백 → gpt-image-1 1회 재시도
  if (!dalle.b64 && /does not exist|model|invalid_value/i.test(dalle.error || '')
      && (process.env.OPENAI_IMAGE_MODEL || '') !== 'gpt-image-1') {
    dalle = await callDallE(apiKey, fullPrompt, 'gpt-image-1');
  }
  if (!dalle.b64) return { url: null, githubPath: null, error: '이미지 생성 실패: ' + (dalle.error || '알 수 없음') };

  // OpenAI 반환 PNG를 바로 저장 (WebP 변환 불필요 — 1024x1024 PNG 품질 충분)
  const imgBuffer = Buffer.from(dalle.b64, 'base64');

  const slug = title
    ? title.replace(/[^\w가-힣]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 60)
    : postId;
  const filename = index === 0 ? `${slug}.png` : `${slug}-${index}.png`;
