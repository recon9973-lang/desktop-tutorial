...
// ── WebP 변환 (WASM, @jsquash) — SEO용. 실패하면 원본(JPG)으로 폴백하므로 안전. ──
// includeFiles 없이 nft 자동 추적에 의존, lock 파일 비커밋. 변환은 컨테이너당 1회 초기화.
let _webpInit = null;
function ensureWebp() {
  if (_webpInit) return _webpInit;
  _webpInit = (async () => {
    const fsp = require('fs').promises;
    const [jpegDec, webpEnc, fd] = await Promise.all([
      import('@jsquash/jpeg/decode.js'),
      import('@jsquash/webp/encode.js'),
      import('wasm-feature-detect'),
    ]);
    await jpegDec.init(await WebAssembly.compile(
      await fsp.readFile(require.resolve('@jsquash/jpeg/codec/dec/mozjpeg_dec.wasm'))));
    const useSimd = await fd.simd();
    await webpEnc.init(await WebAssembly.compile(await fsp.readFile(require.resolve(
      useSimd ? '@jsquash/webp/codec/enc/webp_enc_simd.wasm'
              : '@jsquash/webp/codec/enc/webp_enc.wasm'))));
    return { decodeJpeg: jpegDec.default, encodeWebp: webpEnc.default };
  })().catch((e) => { _webpInit = null; throw e; });
  return _webpInit;
}

async function jpegToWebp(jpegBuffer, quality) {
  const { decodeJpeg, encodeWebp } = await ensureWebp();
...
async function generateAndSaveImage(prompt, postId, index = 0, title = '') {
  ...
  // OpenAI가 준 바이트(gpt-image-1=JPEG). WebP 변환 시도 → 실패 시 원본(JPG)으로 폴백.
  const srcBuffer = Buffer.from(dalle.b64, 'base64');
  let imgBuffer = srcBuffer;
  let ext = detectExt(srcBuffer);
  // SEO용 WebP 변환(JPEG 소스만). IMAGE_WEBP=0 이면 비활성. 실패해도 이미지 유실 없음.
  if (ext === 'jpg' && process.env.IMAGE_WEBP !== '0') {
    try {
      const webp = await jpegToWebp(srcBuffer, Number(process.env.WEBP_QUALITY) || 80);
      if (webp && webp.length > 0) { imgBuffer = webp; ext = 'webp'; }
    } catch (e) {
      console.warn('[image] WebP 변환 실패 → JPG로 저장:', e.message);
    }
  }
