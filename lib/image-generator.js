...
// ── WASM 기반 PNG→WebP 변환 (sharp 네이티브 바이너리 대체) ──
// @jsquash/* 는 ESM·WASM이라 Lambda(Amazon Linux)에서도 안 깨진다.
// 단, 브라우저용이라 기본은 .wasm을 fetch하려다 Node에서 실패 → .wasm을 직접 읽어 주입.
// 초기화는 컨테이너당 1회만(웜 스타트 재사용).
let _webpInitPromise = null;
function ensureWebpEncoder() {
  if (_webpInitPromise) return _webpInitPromise;
  _webpInitPromise = (async () => {
    const [pngDec, webpEnc, featureDetect] = await Promise.all([
      import('@jsquash/png/decode.js'),
      import('@jsquash/webp/encode.js'),
      import('wasm-feature-detect'),
    ]);
    const decodePng = pngDec.default;
    const encodeWebp = webpEnc.default;

    const pngWasm = await WebAssembly.compile(
      await fs.readFile(require.resolve('@jsquash/png/codec/pkg/squoosh_png_bg.wasm')));
    await pngDec.init(pngWasm);

    // @jsquash/webp 내부가 SIMD 지원 여부로 glue를 고르므로, 주입할 .wasm도 동일하게 맞춰야 함.
    const useSimd = await featureDetect.simd();
    const webpWasm = await WebAssembly.compile(await fs.readFile(require.resolve(
      useSimd ? '@jsquash/webp/codec/enc/webp_enc_simd.wasm'
              : '@jsquash/webp/codec/enc/webp_enc.wasm')));
    await webpEnc.init(webpWasm);

    return { decodePng, encodeWebp };
  })().catch((e) => { _webpInitPromise = null; throw e; }); // 실패 시 재시도 가능하게 캐시 비움
  return _webpInitPromise;
}

async function pngToWebp(pngBuffer, quality) {
  const { decodePng, encodeWebp } = await ensureWebpEncoder();
...
async function generateAndSaveImage(prompt, postId, index = 0, title = '') {
  ...
  // OpenAI 반환 PNG → WebP 변환(WASM). 변환 실패 시 PNG로 폴백해 이미지를 절대 유실하지 않음.
  const pngBuffer = Buffer.from(dalle.b64, 'base64');
  let imgBuffer = pngBuffer;
  let ext = 'png';
  try {
    const webp = await pngToWebp(pngBuffer, Number(process.env.WEBP_QUALITY) || 80);
    if (webp && webp.length > 0) { imgBuffer = webp; ext = 'webp'; }
  } catch (e) {
    console.warn('[image] WebP 변환 실패 → PNG로 저장:', e.message);
  }
