...
// 모델별 지원 사이즈로 비율(aspect)을 매핑. dall-e-2는 정사각만 안전.
function sizeFor(model, aspect) {
  if (aspect === 'wide') return model === 'dall-e-3' ? '1792x1024' : model === 'dall-e-2' ? '1024x1024' : '1536x1024';
  if (aspect === 'tall') return model === 'dall-e-3' ? '1024x1792' : model === 'dall-e-2' ? '1024x1024' : '1024x1536';
  return '1024x1024';
}

function callDallE(apiKey, prompt, modelOverride, aspect) {
  ...
  const reqBody = {
    ...
    prompt,
    n: 1,
    size: sizeFor(model, aspect), // 비율→모델별 지원 사이즈
  };
  if (model === 'dall-e-3') {
...
// ── 스타일 프리셋 (원고 스튜디오 이미지 생성 툴용) ──
const STYLE_PRESETS = {
  photo:        'Professional Korean hospital marketing photography, clean modern trustworthy aesthetic, bright soft professional lighting, realistic high quality photo.',
  illustration: 'Clean flat vector illustration, soft pastel healthcare color palette, friendly and warm, minimal rounded shapes, simple background.',
  thumbnail:    'Eye-catching blog thumbnail, clean simple composition, healthcare theme, soft modern colors, generous negative space, crisp and modern.',
  card:         'Korean Instagram card-news style key visual, minimal modern layout, soft healthcare colors, clean and friendly.',
  render3d:     'Soft 3D clay render, healthcare theme, pastel colors, friendly rounded forms, clean studio background, gentle lighting.',
};

// 화면 표시용: GitHub 저장 없이 base64를 바로 반환. { b64, model, mime, error }
async function generateImageB64(prompt, opts = {}) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return { b64: null, model: null, mime: null, error: 'OPENAI_API_KEY 미설정(서버 환경변수)' };
  if (!prompt || !String(prompt).trim()) return { b64: null, model: null, mime: null, error: '프롬프트가 비어 있습니다' };

  const styleText = STYLE_PRESETS[opts.style] || STYLE_PRESETS.photo;
  const fullPrompt = `${styleText} Subject: ${String(prompt).trim()}.${opts.extra ? ' ' + opts.extra : ''} No text overlay, no watermark, no logos, no captions.`;
  const aspect = ['wide', 'tall'].includes(opts.aspect) ? opts.aspect : 'square';

  const configured = process.env.OPENAI_IMAGE_MODEL || 'gpt-image-1';
  const chain = [configured, 'gpt-image-1', 'dall-e-3', 'dall-e-2'].filter((m, i, a) => a.indexOf(m) === i);
  const tried = [];
  for (const model of chain) {
    const r = await callDallE(apiKey, fullPrompt, model, aspect);
    if (r.b64) {
      const buf = Buffer.from(r.b64, 'base64');
      const ext = detectExt(buf);
      return { b64: r.b64, model, mime: ext === 'jpg' ? 'image/jpeg' : 'image/' + ext, error: null };
    }
    tried.push(`${model}: ${r.error || '실패'}`);
  }
  return { b64: null, model: null, mime: null, error: '이미지 생성 실패(모든 모델) — ' + tried.join(' | ') };
}

module.exports = { generateAndSaveImage, generateImageB64, callDallE, STYLE_PRESETS };
