 ...
 */
async function createWpDraft({ siteUrl, user, appPassword, title, html, excerpt = '', status = 'draft', date = '' }) {
  ...
  // 임시저장(draft) / 예약발행(future, date 필요) / 즉시발행(publish)
  let wpStatus = ['draft', 'publish', 'future'].includes(status) ? status : 'draft';
  if (wpStatus === 'future' && !date) wpStatus = 'draft'; // 예약인데 날짜 없으면 안전하게 임시저장
  const payload = { title, content: html, excerpt, status: wpStatus };
  // 예약/즉시발행에 날짜가 있으면 사이트 로컬 시간으로 지정(date: ISO, tz 없음 = 사이트 표준시)
  if (date && (wpStatus === 'future' || wpStatus === 'publish')) payload.date = date;

  const r = await postJSON(endpoint, { Authorization: auth }, payload);
