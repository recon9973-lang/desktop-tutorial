...
### 내가 더 할 수 있는 것 — 오더가 있으면

**「자사 선언 경로」를 사람이 확인하지 않게 만드는 것.** 0.3.558 에 넣은 관문은 **새 선언만**
막고 **이미 저장된 값은 안 고친다**. `sites` 가 이미 아는 사실(한 호스트에 거래처 둘 이상)로
**기존 선언 중 경로 없는 것을 찾아 화면에 띄우는** 자리를 만들 수 있다. 그러면 ②의 한 줄이
사람 손에서 빠진다. **오더 주시면 만든다**(배포는 따로 여쭙는다).
