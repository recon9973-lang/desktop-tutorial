# RESUME (경고색 방) — **셋 다 나갔다** (0.3.612 · 0.3.617 · 0.3.620 · 2026-09-19 14:40 KST 실측)
...
2. **배포 담당을 「만든 방」으로**(0.3.617) — 사장님 «만든 방이 바로 배포해». 네 번째 바뀜이라
   ...
   실패하는 사정까지 §2 에 넣었다.
3. **대기 표에서 이미 나간 다섯 줄을 지움**(0.3.620) — 0.3.616(셋)·0.3.617·0.3.618. 재서 확인한
   것만 지웠다. **0.3.619(검수 방) 줄은 남겼다** — 그때는 도는지 안 쟀다. 누가 재서 지우면 된다.
...
**번호를 세 번 뺏겼다**(0.3.617 시도 → 0.3.619 시도 → 0.3.620 에 나감). 방이 여럿이라 CI 8분
사이에 남이 먼저 닿는다. 거절은 실패가 아니다 — main 부터 본다(§2 에 적힌 그대로).

**합치다 변경이력 구조를 한 번 깨뜨렸다.** 갈래 표시가 항목 한가운데를 갈라, 손으로 이으며
닫는 줄과 쉼표를 먹었다(시험 파일 여덟이 못 읽힘). 그다음부터는 **손으로 잇지 않는다** —
`git show origin/main:apps/web/src/lib/changelog.ts` 를 통째로 가져와 그 위에 내 항목 하나만
얹고, 항목 수·제목을 main 과 대조한다(596 → 597 · 빠진 것 0).
...
- 남이 재서 지우면 되는 것 하나: 대기 표의 **0.3.619(검수 방)** 줄. 0.3.620 이 도는 것을
  14:40 에 확인했으니 그 판도 함께 나갔지만, **안 잰 것을 지우지 않는다**는 원칙대로 남겼다.
