...
5. **대기 표에 이 방 두 줄이 쌓여 있다**(번호 미정 2건) — ①배포 오더 대장에 0.3.587
   줄 되살리기(문서만) ②개체 지도. 가지 `claude/keyword-bubble-cloud` 3커밋
   (`fc2e1af3`·`1ce0e50e`·`784a8a94`). **다음 배포 오더가 있을 때 나간다** —
   「진행해」는 배포 오더가 아니다.
   **나갈 때 할 일 하나**: 변경이력(`changelog.ts`) 줄을 그때 적는다. 미리 적으면
   판 번호가 어긋나 관문 셋이 빨간불이 된다(HISTORY·§2 머리말·APP_VERSION).
