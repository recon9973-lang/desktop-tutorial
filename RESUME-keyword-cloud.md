...
1. **변경이력 번호 고침을 main 에 올린다 — 사장님 판단 대기.**
   커밋 `7fc7e58d`(veo-platform 가지 `claude/keyword-bubble-cloud`)가 변경이력·날짜별
   기록·대장·상태 넷을 **나간 번호 0.3.583** 에 맞춘다. 코드 변경 0.
   까닭은 오류 215 — `git add -A` 로 실패한 배포의 판 번호를 쓸어 담아, 코드는 0.3.583 인데
   변경이력은 0.3.582 로 나갔다(화면 판 ≠ API 판, 0.3.582 항목이 둘).
   ```
   ① 다음 배포에 함께 태운다 (추천 · CI 0분)
   ② 지금 CI 초록 받고 손으로 main 에    ③ 판 하나 더(0.3.584)
   ```
