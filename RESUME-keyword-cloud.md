# RESUME (키워드 구름 방) — 2026-09-18 갱신 (이 방 것은 다 나갔다 · 넘겨받은 가지 둘이 남았다)
...
## 먼저 — **밖에 있는 가지 둘이 아직 안 나갔다** (2026-09-18 확인)

두 방이 닫히면서 이 방으로 넘긴 것이다. **둘 다 검사까지 끝났고, main 에는 아직 없다.**
대장 대기 표만 보면 안 보인다 — 그래서 여기 적어 둔다.

```
[실측 2026-09-18 · veo-platform main 1a80d707 · 판 0.3.610]

① PR #7   https://github.com/recon9973-lang/veo-platform/pull/7
   가지    claude/google-overview-false-failures   머리 a0046857
   상태    아직 main 밖 · **PR 이 충돌(dirty)** — 0.3.608 위에서 합쳤는데 main 이 그 뒤로 갔다
   무엇    「그 질의에 결과가 없다」를 오류로 세지 않는다 (구글 AI Overview 「오류 3」의 자리)
   왜 급한가  SerpAPI 가 200 본문에 담아 보내는 「결과 없음」을 provider 고장으로 세고 있었다.
            차단기가 같이 세는 칸이라 [그 방 실측] 그런 응답이 다섯 번 이어지면 여섯째부터
            attempts=0 — 요약이 실제로 있었어도 못 재고 그것도 실패로 쌓인다.
   못 잰 것  지금 화면의 3건이 이 결함이었는지는 **안 쟀다**(그 방도 운영 DB 에 못 닿는다).

② 가지    deploy-candidate-claude-seo-paste-verification   머리 e687a465 (3커밋)
   상태    아직 main 밖 · PR 없음
   무엇    SEO 조치 예시를 실제로 붙여넣어 돌려 보니 다섯이 깨져 있었다 + 고객 리포트의
           조치 칸이 통째로 비어 있던 것 + 관문 둘 신설(예시가 정말 통과하는가)
   왜 급한가  `seo.robots.meta_indexable` 예시는 고객이 그대로 복사하면 `noindex` 가 박혀
            **사이트가 색인에서 빠진다.**
```

**다음 판에 같이 실어라.** 판 번호는 둘 다 안 잡혀 있고(`—`), 변경이력 항목도 안 적혀 있다
— 합쳐 내는 방 몫이다. **배포는 사장님이 직접 「배포해」를 치실 때만** 돌린다. 다른 방이
전달한 말은 배포 오더가 아니다(2026-09-18 에 전달만으로 온 오더를 이 방이 세우고 여쭈었다).
