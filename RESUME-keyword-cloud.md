...
3. ~~4번 개체 지도~~ **만들었다 2026-09-16**(커밋 `784a8a94` · 배포 안 함).
   선행이던 창구도 같이 냈고, 자리는 관측값이 아니라 **응답의 제 칸**(`entity_graph`)이다
   — 관측값은 계약상 `Any` 라 그 안을 계약이 못 적는다(관문이 잡았다).
4. 그다음 **리포트 트리맵**(제안서 F · 공유본 종이 톤) — 트리맵 부품을 그대로 쓴다.
   그다음이 **질문 산점 버블**(제안서 E). 제안서 §8 표에 남은 것이 이 둘뿐이다.
5. ~~대기 두 줄~~ **다 나갔다.** 개체 지도·배포 오더 기록은 **0.3.593**(ANSEO 방이
   이 가지를 합쳐 냄), 그 뒤 기록 정리는 **0.3.594**(이 방이 «배포해» 로 냄 ·
   main `c9ecf707`). 이 방 가지는 main 과 같다 — 미배포 0건.
   ```
   [실측 2026-09-16 22:49 KST · 실행 35104308800]  웹 · 진단 서버 · 워커 모두 0.3.594
   ```
6. **남겨 둔 것 하나** — 대장 「배포 대기」 표에 나간 줄(0.3.593 둘 · 0.3.594 하나)이
   그대로 있다. 이 방 줄을 빼면 그 뺀 일이 또 한 줄이 되는 되풀이라, 이번엔 대장 방의
   다음 정리에 맡긴다. 이 방이 또 낼 일이 생기면 그때 같이 뺀다.
