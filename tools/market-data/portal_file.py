...
[실측 2026-09-18 · 러너 실행 35325645193] **안 됐다.** 네 데이터셋 모두 소개 페이지조차
200 으로 돌아오지 않았고(8분 동안 매달렸다), 받아진 파일 0개. GitHub 러너에서도
data.go.kr 은 열리지 않는다. 키 없는 길은 여기서 닫혔다 — 자료는 ANSEO 저장소의
`apps/api/data/hira/` 에 이미 있는 분기 파일을 쓴다(`tools/market-data/build_anseo.py`).
